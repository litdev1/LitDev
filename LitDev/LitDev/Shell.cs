...
namespace LitDev
{
    ...
    public static class LDShell
    {
        ...
        // some SELECTED extended properties (german only) for 'GetDetailInfo' Method
        private enum FileDetails
        {
            ...
            kamerahersteller = 32,
            firma = 33,
            dateibeschreibung = 34,
            programmname = 35,
            dauer = 36,
            ist_online = 37,
            periodisch_wiederkehrend = 38,
            ort = 39,
            gesamtgröße = 50,
            kontoname = 51,
            aufgabenstatus = 52,
            computer = 53,
            jahrestag = 54,
            erkennungsmethode = 133,
            anzeigename = 134,
            lokaler_computer = 135,
            hersteller = 136,
            modell = 137,
            paarweise = 138,
            klassifizierung = 139,
            ...
            dateiname = 155,
            dateiversion = 156,
            kennzeichnungsfarbe = 157,
            kennzeichnungsstatus = 158,
            freier_speicherplatz = 159,
            bittiefe = 160,
            horizontale_auflösung = 161,
            breite = 162,
            vertikale_auflösung = 163,
            höhe = 164,
            wichtigkeit = 165,
            anlage = 166,
            ist_gelöscht = 167,
            verschlüsselungsstatus = 168,
            kennzeichnung_vorhanden = 169,
            wurde_beendet = 170,
            unvollständig = 171,
            lesestatus = 172,
            freigegeben = 173,
            ...
            letzter_besuch = 186,
            beschreibung = 187,
            verknüpfungsstatus = 188,
            verknüpfungsziel = 189,
            url = 190,
            medium_erstellt = 191,
            veröffentlichungsdatum = 192,
            codiert_durch = 193,
            produzenten = 194,
            herausgeber = 195,
            an = 215,
            ...
            albuminterpret = 217,
            album_id = 218,
            beats_pro_minute = 219,
            komponisten = 220,
            farbe = 226,
            verwendeter_speicherplatz = 229,
            exif_version = 230,
            aufnahmezeit = 260,
            freigegeben_für = 268,
            freigabestatus = 269,
            ...
            supportlink = 272,
            quelle = 273,
            aufgabenbesitzer = 277,
            gesamtdateigröße = 278,
            marken = 279,
            videokomprimierung = 280,
            regisseure = 281,
            datenrate = 282,
            bildhöhe = 283,
            einzelbildrate = 284,
        ...
        /// <summary>Gets the value of some selected extended properties (GERMAN names only, for internat. s. 'GetDetail') for a given file- or folder.</summary>
        /// <param name="path">The full file- or folder path.</param>
        /// <param name="infoType">The ID number or german name of the property (eg. -1="Infotip"/"", 0="Name", 1="Größe", etc.)
        /// Replace spaces in 'infoType' by '_'.
        ...
        public static Primitive GetDetailInfo(Primitive path, Primitive infoType)
        {
            ...
            string typ = infoType;
            if (String.IsNullOrEmpty(typ)) typ = "Infotip";
            int itNo;	// = -1;
            string strDetail = "";
            ...
            try
            {
                ...
                if (!IsNumber(typ))
                {
                    string tp = typ.Replace(" ", "_");
                    itNo = (int)Enum.Parse(typeof(FileDetails), tp.ToLower());	// gibt zB. 1
                }
                else
                { itNo = Convert.ToInt32(typ); }
        ...
        {
            string scPath = Environment.ExpandEnvironmentVariables(shortcut);
            if (Path.GetExtension(scPath).ToLower() != ".lnk") Path.ChangeExtension(scPath, ".lnk");
        ...
        /// <summary>Gets the total size and -count of all objects (files and folders) in recycle bin (for all drives) as Array. GW must be initialised.</summary>
        /// <param name="drive">The drive name (eg. "C:\"), a path or "" (for all drives).</param>
        /// <returns>A 2element Array with the values for total size [Byte] and -count of all objects in RecBin.</returns>
        public static Primitive GetRecBinSize(Primitive drive)
        {
            string drv = drive;
            if (!string.IsNullOrEmpty(drv)) drv = Path.GetPathRoot(drv);
            Primitive rbData = new Primitive();

            try
            {
                SHQUERYRBINFO sqrbi = new SHQUERYRBINFO();
                sqrbi.cbSize = (uint)Marshal.SizeOf(sqrbi);

                int hresult = SHQueryRecycleBin(drv, ref sqrbi);
                rbData[1] = Convert.ToDouble(sqrbi.i64Size);
                rbData[2] = Convert.ToInt32(sqrbi.i64NumItems);
                return rbData;
            }
            catch
            { return "FAILED"; }
